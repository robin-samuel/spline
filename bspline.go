...
// bspline represents a B-spline.
type bspline struct {
	points []*mat.VecDense
}

// NewBspline creates a new B-spline based on the given points.
func NewBspline(points []Point, ghost bool) (Spline, error) {
	if len(points) < 2 {
	...
	return &bspline{matPoints}, nil
}

func (s *bspline) Len() int {
	return len(s.points)
}

func (s *bspline) At(t float64) Point {
	...
	basisMatrix.Scale(1.0/6.0, basisMatrix)
...
func (s *bspline) Range(start, end, step float64) []Point {
	if start > end {
		start, end = end, start
	}
